...
public static class SweepingLine
{
    public static bool CheckCollision(Polygon p, Polygon q)
    {
        ...
        FastPriorityQueue<VertexEvent>
            vertexEventQueue =
                new(p.Vertices.Length + q.Vertices.Length +
                    1); //Sorted by X coord of the vertex that is the value of the first item of the tuple in the list.
        ...
        while (vertexEventQueue.Count > 0) //Sweep along the X axis
        {
            ...
            if (pIndex != -1) //only one of these should have values unless the polygons have a shared point.
            {
                ...
                var nextPVertex =
                    pVertices[
                        (pIndex + 1) %
                        pVertices
                            .Length]; //remainder division to go through array in a cicular way, would mean PolyLines don't work so need to think about it.
                ...
                if (currentPVertex.X <= previousPVertex.X) slSp.Add((currentPVertex, previousPVertex));
            ...
            if (qIndex != -1) //only one of these should have values unless the polygons have a shared point.
            {
                ...
                if (currentQVertex.X <= previousQVertex.X) slSq.Add((currentQVertex, previousQVertex));
            ...
            {
                slSp.RemoveAll(e =>
                    Math.Abs(e.Item1.X - e.Item2.X) < Constants.Tolerance); //Now remove the vertical edges.
            ...
            for (var i = 0; i < slSp.Count; i++) //Now just check the relevant edges
            {
                ...
                {
                    var qEdge = slSq[j];
                    if (pEdge.LineSegmentIntersection(qEdge)) return true;
                }
            }
