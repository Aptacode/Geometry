...
public static class SweepingLine
{
    public static bool CheckCollision(Polygon p, Polygon q)
    {
        ...
        while (vertexEventQueue.Count > 0) //Sweep along the X axis
        {
            ...
            {
                slSp.RemoveAll(e =>
                    Math.Abs(e.P1.X - e.P2.X) < Constants.Tolerance); //Now remove the vertical edges.
                slSq.RemoveAll(e => Math.Abs(e.P1.X - e.P2.X) < Constants.Tolerance);
                continue;
            }
            ...
            slSp.RemoveAll(e => Math.Abs(e.P1.X - e.P2.X) < Constants.Tolerance); //Now remove the vertical edges.
            slSq.RemoveAll(e => Math.Abs(e.P1.X - e.P2.X) < Constants.Tolerance);
        }
