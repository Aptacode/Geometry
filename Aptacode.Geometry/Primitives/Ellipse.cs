...
namespace Aptacode.Geometry.Primitives
{
    public record Ellipse : Primitive
    {
        ...
        private (Vector2, Vector2) GetFoci()
        {
            if(Radii.X > Radii.Y)
            {
                var c = Vector2.Transform(new Vector2((float)Math.Sqrt(Radii.X * Radii.X - Radii.Y * Radii.Y), 0.0f), Matrix3x2.CreateRotation(Rotation));
                var f1 = Position - c;
                var f2 = Position + c;
                return (f1, f2);
            }
            ...
            {
                var c = Vector2.Transform(new Vector2(0.0f, (float)Math.Sqrt(Radii.Y * Radii.Y - Radii.X * Radii.X)), Matrix3x2.CreateRotation(Rotation));
                var f1 = Position - c;
                var f2 = Position + c;
                return (f1, f2);
            }
            else //The radii are the same and this is a circle
            {
        ...
        public static Ellipse Create(float x, float y, float a, float b, float rotation) => new(new Vector2(x, y), new Vector2(a, b), rotation);
        ...
        public static (double u0, double u1, double u2, double u3, double u4) GetResultantPolynomial(double A1, double B1, double C1, double D1, double E1, double F1, double A2, double B2, double C2, double D2, double E2, double F2) //Takes the coefficients of 2 ellipses (conics) and returns the Bezout determinant R(y) = u0 + u1y + u2y^2 + u3y^3 + u4y^4, is is the equation for the solutions for the y points of intersection, they may be complex.
        {
            var v0 = A1 * B2 - A2 * B1;
            var v1 = A1 * C2 - A2 * C1;
            var v2 = A1 * D2 - A2 * D1;
